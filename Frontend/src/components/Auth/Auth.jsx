...
const Transition = ({setLoginActive, set, name})=>(
  <div className="container grid grid-rows-6 grid-flow-col content-center gap-4 px-5">
  ...
  <button className="bg-transparent border-2 border-white hover:bg-white hover:text-emerald-500 text-white font-bold py-2 px-10 rounded w-25" onClick={()=>setLoginActive(set)}>
    {name}
  </button>
  </div>
</div>
...
export default function Auth() {
  ...
  return (
    <div className="grid grid-cols-6 h-screen ">
      <div className={`${loginActive? 'bg-emerald-500 col-span-2': 'bg-white col-span-4'}`}>
        {loginActive ? (
